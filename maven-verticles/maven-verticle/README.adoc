...
To run the fat jar:
    ...
    java -jar target/maven-verticle-3.5.4-fat.jar
...
want to utilize them all, you can deploy 8 instances as follows:
    ...
    java -jar target/maven-verticle-3.5.4-fat.jar -instances 8

You can also enable clustering and ha at the command line, e.g.

    java -jar target/maven-verticle-3.5.4-fat.jar -cluster

    java -jar target/maven-verticle-3.5.4-fat.jar -ha

You can also pass a configuration to the verticle:

    java -jar target/maven-verticle-3.5.4-fat.jar -conf src/conf/my-conf.json
